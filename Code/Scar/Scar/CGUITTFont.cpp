...
void CGUITTFont::draw(const core::stringw& text, const core::rect<s32>& position, video::SColor color, bool hcenter, bool vcenter, const core::rect<s32>* clip)
{
	...
	while (!j.atEnd())
	{
		...
		{
			u32 kk = page->render_positions.size();
			for ( u32 qq = 0; qq < kk; qq ++)
				page->render_positions[qq] += core::vector2d<s32>(15,15);*/
		s32 posnum = page->render_positions.size();
		core::array<core::myposition> positions;
		for ( s32 k = 0; k < posnum; k++)
		{
			core::myposition posnode;
			posnode.LeftUpper = page->render_positions[k];
			posnode.RightUpper = posnode.LeftUpper + core::vector2d<s32>( page->render_source_rects[k].getSize().Width, 0 );
			posnode.RightLower = posnode.LeftUpper + core::vector2d<s32>( page->render_source_rects[k].getSize() );
			posnode.LeftLower = posnode.LeftUpper + core::vector2d<s32>( 0, page->render_source_rects[k].getSize().Height );


			positions.push_back( posnode );
		}
		//Driver->draw2DImageBatch(page->texture, page->render_positions, page->render_source_rects, clip, color, true);
		Driver->mydraw2DImageBatch(page->texture, positions, page->render_source_rects, clip, color, true);
		//}
	}
}
