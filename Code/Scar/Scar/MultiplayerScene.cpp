...
{				
			...
			if ( bRunOnce )
			{
				...
				bfEarth->addAnimator( relStay );
				relStay->drop();
				relStay = pEngine->GetMySceneManager()->createRelateCameraStayAnimator(
					0, 1000, m_pCamera, bfMoon->getPosition() );
			...
			else if ( SubState == 1 )
			{
				...
				dynamic_cast<MyEventReceiver*>( MyIrrlichtEngine::pEventReceiver )->SetEventCallbackFunc(
					[ this, gui, box, uiManager ]( const SEvent& event )->void*
				{	
					m_playerManager->OnEvent( event );
