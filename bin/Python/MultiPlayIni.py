...
def GetRoot():
    ...
    # 速度槽空
    speedBar2 = uiManager.AddUIImage( root, 80, 456, 0, 0, vector2df( cenX + 446, cenY ) )
    speedBar2.LoadImage("../media/UIResource/Game/speed_2.png")
    speedBar2.SetName( "speed2" )
    speedBar2.SetAlpha( 0 )
    speedBar2.SetVisible( False )
    # 速度槽满
    speedBar1 = uiManager.AddUIImage( root, 80, 456, 0, 0, vector2df( cenX + 446, cenY ) )
    speedBar1.LoadImage("../media/UIResource/Game/speed_1.png")
    speedBar1.SetName( "speed1" )
    speedBar1.SetAlpha( 0 )
    ...
    speedText.SetPosition( vector2df( cenX + 330, cenY + 210 ) )
    speedText.SetVisible( False )

    # 能量槽空
    energy2 = uiManager.AddUIImage( root, 78, 437, 0, 0, vector2df( cenX + 426, cenY ) )
    energy2.LoadImage("../media/UIResource/Game/energy_2.png")
    energy2.SetName( "energy2" )
    energy2.SetAlpha( 0 )
    energy2.SetVisible( False )
    # 能量槽满
    energy1 = uiManager.AddUIImage( root, 78, 437, 0, 0, vector2df( cenX + 426, cenY ) )
    ...
    energyText.SetPosition( vector2df( cenX + 345, cenY + 190 ) )
    energyText.SetVisible( False )

    # 护盾槽空
    shield2 = uiManager.AddUIImage( root, 78, 437, 0, 0, vector2df( cenX - 426, cenY ) )
    shield2.LoadImage("../media/UIResource/Game/shield_2.png")
    shield2.SetName( "shield2" )
    shield2.SetAlpha( 0 )
    shield2.SetVisible( False )
    # 护盾槽满
    shield1 = uiManager.AddUIImage( root, 78, 437, 0, 0, vector2df( cenX - 426, cenY ) )
    ...
    shieldText.SetPosition( vector2df( cenX - 315, cenY + 190 ) )
    shieldText.SetVisible( False )

    # 护甲槽满
    armor2 = uiManager.AddUIImage( root, 80, 456, 0, 0, vector2df( cenX - 446, cenY ) )
    armor2.LoadImage("../media/UIResource/Game/armor_2.png")
    armor2.SetName( "armor2" )
    armor2.SetAlpha( 0 )
    armor2.SetVisible( False )
    # 护甲槽满
    armor1 = uiManager.AddUIImage( root, 80, 456, 0, 0, vector2df( cenX - 446, cenY ) )
    ...
    armorText.SetPosition( vector2df( cenX - 300, cenY + 210 ) )
    ...
    target2.SetAntiAliasing()
    target2.SetVisible( False )

    #锁定框——已锁定
    lock = uiManager.AddUIImage( root, 100, 100 )
    lock.SetPosition( vector2df( 200, 200 ) )
    lock.SetName( "lock1" )
    lock.SetVisible( False )
    '''lock1 = uiManager.AddUIImage( lock, 100, 100 )
    lock1.LoadImage( "../media/UIResource/Game/lock1.png" )
    lock1.SetAntiAliasing()
    ani = uiManager.CreateAnimatorRotation( 0, 2000, 360, True )
    lock1.AddAnimator( ani )
    ani.drop()'''
    lock2 = uiManager.AddUIImage( lock, 100, 100 )
    lock2.LoadImage( "../media/UIResource/Game/lock2.png" )
    lock2.SetAntiAliasing()
    ani = uiManager.CreateAnimatorRotation( 0, 2000, -360, True )
    lock2.AddAnimator( ani )
    ani.drop()
    #敌人详细信息
    enemyInfo = uiManager.AddUIImage( lock, 150, 80 )
    enemyInfo.LoadImage( "../media/UIResource/Game/target_info_box.png" )
    enemyInfo.SetAntiAliasing()
    enemyInfo.SetPosition( vector2df( 105, 0 ) )
    #enemyInfo.SetName( "enemyInfo" )
    distance = uiManager.AddUIStaticText( enemyInfo, 100, 20, u"距离：", SColor(255,255,255,255) )
    distance.SetName( "targetDistance" )
    distance.SetPosition( vector2df( 40, 23 ) )
    shield = uiManager.AddUIStaticText( enemyInfo, 100, 20, u"护盾：", SColor(255,255,255,255) )
    shield.SetName( "targetShield" )
    shield.SetPosition( vector2df( 40, 0 ) )
    armor = uiManager.AddUIStaticText( enemyInfo, 100, 20, u"护甲：", SColor(255,255,255,255) )
    armor.SetName( "targetArmor" )
    armor.SetPosition( vector2df( 40, -23 ) )
    ...
    #全屏幕红色
    redMaskCtrl = uiManager.AddUIImage( root, 0, 0 )
    redMaskCtrl.SetName( "redMaskCtrl" )
    redMaskCtrl.SetVisible( False )
    redMask = uiManager.AddUIImage( redMaskCtrl, ScreenWidth, ScreenHeight, 0, 1, vector2df( cenX, cenY ) )
